...
def check_dir(input_path):
    ...
    for i in input_path:
        if os.path.exists(i):
            logger.info('folder {0} already exists'.format(i))
        else:
            os.mkdir(i)
...
def trans_imgs(input_path, output_path):
    """对图像进行一系列操作，增强数据集
...
if __name__ == '__main__':
    ...
    trans_imgs(input_cataract_path, output_cataract_path)
    trans_imgs(input_normal_path, output_normal_path)
    end = time.time()
